...
- diff showNumbersicult to understand
- longer to debug
- hard to maintain
- tricky to extend
...
```js showNumbersx showNumbers title="Example 2"
const ViewToolbar = ({
  onRefresh,
  lastUpdatedTime,
  actionContent,
...
```js showNumbersx showNumbers title="By alpha sorting"
const ViewToolbar = ({
  actionContent,
  filterContent,
  filterOpen,
  ...
  return !loading ? (
    <div>
      {Boolean(error)
        ...
        : <DataList data={data} onItemShowDetails={showDialog} />
      }
...
```js showNumbers title="By using early return in jsx showNumbers"
const DataPreview = () => {
  const [loading, setLoading] = React.useState(false);
  const [data, setData] = React.useState([]);
  const [error, setError] = React.useState(null);
